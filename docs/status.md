...
| Distribution | Preview released; MSIX foundation verified | `0.9.0` is published as a GitHub pre-release with an Actions-built portable x64 ZIP, SHA-256 checksum, and full MIT text as `LICENSE.txt`. A parameterized `packagedClassicApp` manifest, Store logo set, MakeAppx script, and manual Actions workflow are implemented. Local and GitHub-hosted builds produced valid x64 MSIX and `.msixupload` archives with `.appxsym`/PDB crash symbols using repository secrets. Test signing, installation, WACK, and Partner Center validation remain. `1.0.0`/MSIX `1.0.0.0` is the first stable Store target; Store product name `GdW CaptureView` is reserved |
